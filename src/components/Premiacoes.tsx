import styles from '../style/premiacoes.module.css'

export default function Premiacoes(){
 return(
  <div className={styles.premiacoesContainer}>
   <h2>Premiações</h2>
   <div className={styles.premiacoes}>
    <div className={styles.premiacao}>
     <div className={styles.logoPremiacao}>
     <svg width="24" height="33" viewBox="0 0 24 33" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M23.8379 2.57479L17.8919 0.196411L13.2432 11.6559C15.5135 11.8721 17.5675 12.8451 19.1892 14.2505L23.8379 2.57479Z" fill="#2F7931"/>
      <path d="M10.8648 11.6559L6.21622 0.196411L0.16217 2.57479L4.91893 14.2505C6.54052 12.8451 8.5946 11.9802 10.8648 11.6559Z" fill="#2F7931"/>
      <path d="M12.0541 12.7369C6.64867 12.7369 2.32434 17.0613 2.32434 22.4667C2.32434 27.8721 6.64867 32.1964 12.0541 32.1964C17.4595 32.1964 21.7838 27.8721 21.7838 22.4667C21.7838 17.0613 17.3514 12.7369 12.0541 12.7369ZM15.8378 27.8721L12.054 25.3856L8.27026 27.8721L9.89191 23.5478L6.64867 21.3856H10.4325L12.0541 17.0613L13.6758 21.3856H17.4596L14.2163 23.5478L15.8378 27.8721Z" fill="#2F7931"/>
      </svg>
     </div>
     <div className={styles.informationPremiacao}>
      <h3>Título de Utilidade Pública Municipal – Crato/CE</h3>
      <p>Em 18 de agosto de 2021, a Instituição UPAMA foi declarada de utilidade pública pela Prefeitura Municipal de Crato, através da Lei nº 3.809/2021.</p>
     </div>
    </div>
    <div className={styles.premiacao}>
     <div className={styles.logoPremiacao}>
      <svg width="24" height="33" viewBox="0 0 24 33" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M23.8379 2.57479L17.8919 0.196411L13.2432 11.6559C15.5135 11.8721 17.5675 12.8451 19.1892 14.2505L23.8379 2.57479Z" fill="#2F7931"/>
      <path d="M10.8648 11.6559L6.21622 0.196411L0.16217 2.57479L4.91893 14.2505C6.54052 12.8451 8.5946 11.9802 10.8648 11.6559Z" fill="#2F7931"/>
      <path d="M12.0541 12.7369C6.64867 12.7369 2.32434 17.0613 2.32434 22.4667C2.32434 27.8721 6.64867 32.1964 12.0541 32.1964C17.4595 32.1964 21.7838 27.8721 21.7838 22.4667C21.7838 17.0613 17.3514 12.7369 12.0541 12.7369ZM15.8378 27.8721L12.054 25.3856L8.27026 27.8721L9.89191 23.5478L6.64867 21.3856H10.4325L12.0541 17.0613L13.6758 21.3856H17.4596L14.2163 23.5478L15.8378 27.8721Z" fill="#2F7931"/>
      </svg>
     </div>
     <div className={styles.informationPremiacao}>
      <h3>Título de Utilidade Pública Estadual – Ceará</h3>
      <p>Através da Lei Nº 18.192, de 31 de Agosto de 2022, o Governo do Estado do Ceará declarou a Instituição UPAMA com o título de Utilidade Pública Estadual.</p>
     </div>
    </div>
   </div>
  </div>
 )
}